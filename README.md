...
**Импорт папок с AI-анализом** — перетащите папку с файлами в боковое меню, и AI автоматически создаст структуру wiki-статей:
- Автоматическое распознавание содержимого файлов
- Генерация заголовков и тегов через AI
- Конвертация в Markdown с форматированием
- Сохранение иерархии папок в структуре страниц
...
Проект использует **pgvector** — расширение PostgreSQL для хранения и поиска векторных представлений (embeddings).

| Компонент | Описание |
|-----------|------------|
| **Docker образ** | `pgvector/pgvector:pg17` |
| **Размерность** | 384 измерения (nomic-embed-text) |
| **Индекс** | IVFFlat для быстрого поиска |
| **Метрика** | Cosine similarity |

**Преимущества pgvector:**
- Встроенная поддержка векторных операций в SQL
- Автоматическое индексирование для ускорения поиска
- Совместимость с существующими PostgreSQL инструментами
- Нет необходимости в отдельной векторной БД (Pinecone, Weaviate)
...
**Рекомендуемый индекс для >10k записей:**
```sql
...
### Batch-индексация Embeddings

Система поддерживает массовую генерацию embeddings для всех страниц wiki:

| Параметр | Значение | Описание |
|-----------|----------|------------|
| **Batch Size** | 10 страниц | Количество страниц в одном батче |
| **Concurrency** | 3 потока | Параллельная обработка |
| **Retry** | 3 попытки | Повторы при ошибках |
| **Delay** | 1 секунда | Задержка между батчами |

**API для управления:**

| Endpoint | Описание |
|----------|------------|
| `ai.batchStart` | Запуск индексации |
| `ai.batchStop` | Остановка индексации |
| `ai.batchResume` | Возобновление индексации |
| `ai.batchProgress` | Прогресс индексации |
| `ai.embeddingsStats` | Статистика покрытия |

**Функции:**
- Автоматический пропуск уже проиндексированных страниц
- Использование Redis кэша для ускорения
...
Для production-среды рекомендуется использовать Redis Sentinel:
...
**Архитектура:**
- 1 Master + 2 Replica
- 3 Sentinel для мониторинга
- Автоматический failover при падении master
- Quorum: 2 голоса для переключения
...
### Мониторинг Redis с Grafana

Система включает полный стек мониторинга:
...
| **Redis Exporter** | 9121 | Экспорт метрик Redis |
| **Alertmanager** | 9093 | Управление алертами |

**Grafana дашборд включает:**
- Hit Rate кэша (% успешных запросов)
...
## Импорт папок (Drag & Drop)

Система поддерживает импорт целых папок с файлами через drag-and-drop:

### Возможности
- **Drag & Drop импорт** — перетащите папку в боковое меню wiki
- **Сохранение структуры** — вложенные папки становятся подстраницами
- **AI-анализ** — автоматическое распознавание содержимого файлов
- **Параллельная обработка** — до 5 файлов одновременно

### Поддерживаемые форматы
| Формат | Обработка |
|--------|------------|
| `.md`, `.txt` | Прямой импорт текста |
| `.pdf`, `.docx`, `.xlsx` | AI-анализ содержимого |
| `.jpg`, `.png`, `.webp` | AI-распознавание изображений |
| Папки | Создание родительских страниц |

### Использование
1. Откройте wiki
2. Перетащите папку в боковое меню
3. Выберите родительскую страницу (опционально)
4. Включите/отключите AI-анализ
